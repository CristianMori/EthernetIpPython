"""CIP Safety CRC computation routines.

Five algorithms with precomputed lookup tables.

Runtime CRCs (S1, S2, S3, S5) are seeded by the Producer/Consumer
Identifier (PID/CID). Configuration CRCs (S4) use fixed seeds (0xFFFFFFFF).

Lookup tables match the HMS/IXXAT CSS reference implementation. Verified
byte-for-byte against the C# implementation.
"""
from __future__ import annotations
import struct

# ==================== CRC-S1 (8-bit, poly 0x37) ====================

_S1_TABLE = (
    ,
)


# ==================== CRC-S2 (8-bit, poly 0x3B) ====================

_S2_TABLE = (
    ,
)


# ==================== CRC-S3 (16-bit, poly 0x080F) ====================

_S3_TABLE = (
    0x0000, 0x080F, 0x101E, 0x1811, 0x203C, 0x2833, 0x3022, 0x382D,
    0x4078, 0x4877, 0x5066, 0x5869, 0x6044, 0x684B, 0x705A, 0x7855,
    0x80F0, 0x88FF, 0x90EE, 0x98E1, 0xA0CC, 0xA8C3, 0xB0D2, 0xB8DD,
    0xC088, 0xC887, 0xD096, 0xD899, 0xE0B4, 0xE8BB, 0xF0AA, 0xF8A5,
    0x09EF, 0x01E0, 0x19F1, 0x11FE, 0x29D3, 0x21DC, 0x39CD, 0x31C2,
    0x4997, 0x4198, 0x5989, 0x5186, 0x69AB, 0x61A4, 0x79B5, 0x71BA,
    0x891F, 0x8110, 0x9901, 0x910E, 0xA923, 0xA12C, 0xB93D, 0xB132,
    0xC967, 0xC168, 0xD979, 0xD176, 0xE95B, 0xE154, 0xF945, 0xF14A,
    0x13DE, 0x1BD1, 0x03C0, 0x0BCF, 0x33E2, 0x3BED, 0x23FC, 0x2BF3,
    0x53A6, 0x5BA9, 0x43B8, 0x4BB7, 0x739A, 0x7B95, 0x6384, 0x6B8B,
    0x932E, 0x9B21, 0x8330, 0x8B3F, 0xB312, 0xBB1D, 0xA30C, 0xAB03,
    0xD356, 0xDB59, 0xC348, 0xCB47, 0xF36A, 0xFB65, 0xE374, 0xEB7B,
    0x1A31, 0x123E, 0x0A2F, 0x0220, 0x3A0D, 0x3202, 0x2A13, 0x221C,
    0x5A49, 0x5246, 0x4A57, 0x4258, 0x7A75, 0x727A, 0x6A6B, 0x6264,
    0x9AC1, 0x92CE, 0x8ADF, 0x82D0, 0xBAFD, 0xB2F2, 0xAAE3, 0xA2EC,
    0xDAB9, 0xD2B6, 0xCAA7, 0xC2A8, 0xFA85, 0xF28A, 0xEA9B, 0xE294,
    0x27BC, 0x2FB3, 0x37A2, 0x3FAD, 0x0780, 0x0F8F, 0x179E, 0x1F91,
    0x67C4, 0x6FCB, 0x77DA, 0x7FD5, 0x47F8, 0x4FF7, 0x57E6, 0x5FE9,
    0xA74C, 0xAF43, 0xB752, 0xBF5D, 0x8770, 0x8F7F, 0x976E, 0x9F61,
    0xE734, 0xEF3B, 0xF72A, 0xFF25, 0xC708, 0xCF07, 0xD716, 0xDF19,
    0x2E53, 0x265C, 0x3E4D, 0x3642, 0x0E6F, 0x0660, 0x1E71, 0x167E,
    0x6E2B, 0x6624, 0x7E35, 0x763A, 0x4E17, 0x4618, 0x5E09, 0x5606,
    0xAEA3, 0xA6AC, 0xBEBD, 0xB6B2, 0x8E9F, 0x8690, 0x9E81, 0x968E,
    0xEEDB, 0xE6D4, 0xFEC5, 0xF6CA, 0xCEE7, 0xC6E8, 0xDEF9, 0xD6F6,
    0x3462, 0x3C6D, 0x247C, 0x2C73, 0x145E, 0x1C51, 0x0440, 0x0C4F,
    0x741A, 0x7C15, 0x6404, 0x6C0B, 0x5426, 0x5C29, 0x4438, 0x4C37,
    0xB492, 0xBC9D, 0xA48C, 0xAC83, 0x94AE, 0x9CA1, 0x84B0, 0x8CBF,
    0xF4EA, 0xFCE5, 0xE4F4, 0xECFB, 0xD4D6, 0xDCD9, 0xC4C8, 0xCCC7,
    0x3D8D, 0x3582, 0x2D93, 0x259C, 0x1DB1, 0x15BE, 0x0DAF, 0x05A0,
    0x7DF5, 0x75FA, 0x6DEB, 0x65E4, 0x5DC9, 0x55C6, 0x4DD7, 0x45D8,
    0xBD7D, 0xB572, 0xAD63, 0xA56C, 0x9D41, 0x954E, 0x8D5F, 0x8550,
    0xFD05, 0xF50A, 0xED1B, 0xE514, 0xDD39, 0xD536, 0xCD27, 0xC528,
)


def _build_s4_table() -> tuple[int, ...]:
    table = []
    for i in range(256):
        crc = i
        for _ in range(8):
            crc = (crc >> 1) ^ 0xEDB88320 if (crc & 1) else (crc >> 1)
        table.append(crc & 0xFFFFFFFF)
    return tuple(table)


def _build_s5_table() -> tuple[int, ...]:
    POLY = 0x5D6DCB
    table = []
    for i in range(256):
        crc = i << 16
        for _ in range(8):
            if crc & 0x800000:
                crc = ((crc << 1) ^ POLY) & 0xFFFFFFFF
            else:
                crc = (crc << 1) & 0xFFFFFFFF
        table.append(crc & 0x00FFFFFF)
    return tuple(table)


_S4_TABLE = _build_s4_table()
_S5_TABLE = _build_s5_table()


class SafetyCrc:
    """Namespace class — all methods are @staticmethod for parity with the C# layout."""

    @staticmethod
    def compute_s1(data: bytes, preset: int) -> int:
        """8-bit CRC-S1 (poly 0x37). Actual data CRC + timestamp CRC."""
        crc = preset & 0xFF
        for b in data:
            crc = _S1_TABLE[crc ^ b]
        return crc

    @staticmethod
    def compute_s2(data: bytes, preset: int) -> int:
        """8-bit CRC-S2 (poly 0x3B). Complement data CRC in base-format short packets."""
        crc = preset & 0xFF
        for b in data:
            crc = _S2_TABLE[crc ^ b]
        return crc

    @staticmethod
    def compute_s3(data, preset: int) -> int:
        """16-bit CRC-S3 (poly 0x080F).

        Accepts: bytes-like, single int (one byte), or a 16-bit int handled as
        little-endian byte pair.
        """
        crc = preset & 0xFFFF
        if isinstance(data, int):
            if 0 <= data <= 0xFF:
                return ((_S3_TABLE[((crc >> 8) ^ data) & 0xFF]) ^ ((crc << 8) & 0xFFFF)) & 0xFFFF
            # treat as 16-bit LE
            lo = data & 0xFF
            hi = (data >> 8) & 0xFF
            crc = ((_S3_TABLE[((crc >> 8) ^ lo) & 0xFF]) ^ ((crc << 8) & 0xFFFF)) & 0xFFFF
            crc = ((_S3_TABLE[((crc >> 8) ^ hi) & 0xFF]) ^ ((crc << 8) & 0xFFFF)) & 0xFFFF
            return crc
        for b in data:
            crc = ((_S3_TABLE[((crc >> 8) ^ b) & 0xFF]) ^ ((crc << 8) & 0xFFFF)) & 0xFFFF
        return crc

    @staticmethod
    def compute_s4(data: bytes, preset: int = 0xFFFFFFFF) -> int:
        """32-bit CRC-S4 (CRC-32 reflected). CPCRC, SCCRC, config CRCs."""
        crc = preset & 0xFFFFFFFF
        for b in data:
            crc = _S4_TABLE[(crc ^ b) & 0xFF] ^ (crc >> 8)
        return crc & 0xFFFFFFFF

    @staticmethod
    def compute_s5(data: bytes, preset: int) -> int:
        """24-bit CRC-S5 (poly 0x5D6DCB) with XorOut applied — final wire value."""
        return SafetyCrc.compute_s5_raw(data, preset) ^ 0x00FFFFFF

    @staticmethod
    def compute_s5_raw(data: bytes, preset: int) -> int:
        """24-bit CRC-S5 without XorOut — use for seeds that feed further CRC steps."""
        crc = preset & 0x00FFFFFF
        for b in data:
            crc = _S5_TABLE[((crc >> 16) ^ b) & 0xFF] ^ ((crc << 8) & 0x00FFFFFF)
        return crc

    # ==================== PID/CID Seed Computation ====================

    @staticmethod
    def pid_cid_seed_s1(vendor_id: int, device_serial: int, connection_serial: int) -> int:
        buf = struct.pack('<HIH', vendor_id, device_serial, connection_serial)
        return SafetyCrc.compute_s1(buf, 0)

    @staticmethod
    def pid_cid_seed_s3(vendor_id: int, device_serial: int, connection_serial: int) -> int:
        buf = struct.pack('<HIH', vendor_id, device_serial, connection_serial)
        return SafetyCrc.compute_s3(buf, 0)

    @staticmethod
    def pid_cid_seed_s5(vendor_id: int, device_serial: int, connection_serial: int) -> int:
        """Returns raw (no XorOut) seed for chained S5 computation."""
        buf = struct.pack('<HIH', vendor_id, device_serial, connection_serial)
        return SafetyCrc.compute_s5_raw(buf, 0)

    @staticmethod
    def pid_rollover_seed_s3(rollover_count: int, pid_seed_s3: int) -> int:
        return SafetyCrc.compute_s3(rollover_count & 0xFFFF, pid_seed_s3)

    @staticmethod
    def pid_rollover_seed_s5(rollover_count: int, pid_seed_s5: int) -> int:
        buf = struct.pack('<H', rollover_count & 0xFFFF)
        return SafetyCrc.compute_s5_raw(buf, pid_seed_s5)
